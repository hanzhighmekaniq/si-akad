<x-admin-layout>
    <x-slot name="title">Manajemen Golongan - SI Akademik</x-slot>

    <div class="mb-6">
        <h2 class="text-3xl font-bold text-gray-900">Manajemen Golongan</h2>
        <p class="mt-1 text-sm text-gray-600">Kelola data golongan/kelas mahasiswa</p>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-6">
        <form method="GET" action="{{ route('admin.golongan.index') }}" class="flex gap-4">
            <!-- Search Input -->
            <div class="flex-1">
                <label for="search" class="block mb-2 text-sm font-medium text-gray-900">Cari Golongan</label>
                <input type="text" id="search" name="search" value="{{ request('search') }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                    placeholder="Cari ID atau nama golongan...">
            </div>
    ...
    <div class="flex items-center justify-between mb-4">
        <div>
            <p class="text-sm text-gray-600">Total: <span class="font-semibold">{{ $golongan->total() }}</span> golongan</p>
        </div>
            ...
            <table class="w-full text-sm text-left text-gray-500">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3">ID Golongan</th>
                        <th scope="col" class="px-6 py-3">Nama Golongan</th>
                        <th scope="col" class="px-6 py-3 text-center">Jumlah Mahasiswa</th>
                ...
                <tbody>
                    @forelse ($golongan as $gol)
                        <tr class="bg-white border-b hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                <span class="font-mono font-semibold text-gray-900">{{ $gol->id_Gol }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="font-medium text-gray-900">{{ $gol->nama_Gol }}</span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                @if($gol->mahasiswa_count > 0)
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-blue-100 text-blue-800">
                                        {{ $gol->mahasiswa_count }} Mahasiswa
                                    </span>
                                @else
                            ...
                            <td class="px-6 py-4 text-center">
                                @if($gol->jadwal_akademik_count > 0)
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-green-100 text-green-800">
                                        {{ $gol->jadwal_akademik_count }} Jadwal
                                    </span>
                                @else
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-gray-100 text-gray-800">
                                        Belum ada
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <form action="{{ route('admin.golongan.destroy', $gol->id_Gol) }}" method="POST"
                                        onsubmit="return confirm('Yakin ingin menghapus golongan {{ $gol->nama_Gol }}?')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-8 text-center text-gray-500">
                                ...
                                <p class="text-lg font-medium">Tidak ada data golongan</p>
                                <p class="text-sm">Tambahkan golongan baru untuk memulai</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
