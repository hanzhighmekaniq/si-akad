<x-admin-layout>
    <x-slot name="title">Edit Ruangan - SI Akademik</x-slot>
    ...
    <div class="mb-6">
        <div class="flex items-center gap-2 mb-2">
            ...
            <h2 class="text-3xl font-bold text-gray-900">Edit Ruangan</h2>
        </div>
        <p class="mt-1 text-sm text-gray-600">Update informasi ruangan: {{ $ruang->nama_ruang }}</p>
    </div>

    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <form method="POST" action="{{ route('admin.ruang.update', $ruang->id_ruang) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="id_ruang" id="id_ruang" value="{{ old('id_ruang', $ruang->id_ruang) }}"
                ...
                <div class="p-4 text-sm text-yellow-800 rounded-lg bg-yellow-50 border border-yellow-200" role="alert">
                    <div class="flex items-start">
                        ...
                        <div>
                            ...
                            <p class="mt-1">Ruangan ini sedang digunakan dalam {{ $ruang->jadwalAkademik()->count() }} jadwal akademik. Perubahan ID ruangan dapat mempengaruhi data jadwal yang ada.</p>
                        </div>
                    </div>
                </div>
            @endif
