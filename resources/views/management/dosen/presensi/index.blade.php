<x-admin-layout>
    ...
    <div class="mb-4 p-4 bg-gray-50 border border-gray-200 rounded-lg">
        <h3 class="text-sm font-semibold text-gray-900 mb-2">Penanda status presensi</h3>
        <ul class="flex flex-wrap gap-4 text-xs text-gray-700">
            <li class="inline-flex items-center gap-1.5">
                <span class="inline-block w-3 h-3 rounded bg-green-200 border border-green-300"></span>
                <span><strong>Sudah diabsen</strong> — Presensi untuk pertemuan hari ini sudah diinput oleh dosen.</span>
            </li>
            <li class="inline-flex items-center gap-1.5">
                <span class="inline-block w-3 h-3 rounded bg-amber-200 border border-amber-300"></span>
                <span><strong>Jadwal sudah lewat — belum diinput</strong> — Waktu kuliah sudah lewat, presensi belum dicatat.</span>
            </li>
            <li class="inline-flex items-center gap-1.5">
                <span class="inline-block w-3 h-3 rounded bg-gray-200 border border-gray-300"></span>
                <span><strong>Presensi belum diinput</strong> — Belum ada data kehadiran untuk pertemuan ini.</span>
            </li>
        </ul>
    </div>
                        ...
                        @foreach ($jadwalHariIni as $jadwal)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="space-y-2">
                                        @php
                                            $sudahInput = $presensiSudahInput[$jadwal->id] ?? false;
                                            $sudahLewat = $jadwalSudahLewat[$jadwal->id] ?? false;
                                        @endphp
                                        @if($sudahInput)
                                        ...
                                        @elseif($sudahLewat)
                                            <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-amber-100 text-amber-800 border border-amber-200">
                                                <svg class="w-3.5 h-3.5 mr-1" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92z" clip-rule="evenodd"/></svg>
                                                Jadwal sudah lewat — presensi belum diinput
                                            </span>
                                        @else
                                    ...
                                    <div class="flex items-center justify-center gap-2 mt-2">
                                        <a href="{{ route('dosen.presensi.create', $jadwal->id) }}"
                                            class="inline-flex items-center px-3 py-1.5 {{ $sudahInput ? 'bg-gray-500 hover:bg-gray-600' : 'bg-green-600 hover:bg-green-700' }} text-white text-xs font-medium rounded-lg transition">
                                            <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clip-rule="evenodd" />
                                            </svg>
                                            {{ $sudahInput ? 'Edit Presensi' : 'Input Presensi' }}
                                        </a>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            @foreach ($semuaJadwal[$hari] as $jadwal)
                                <div class="border border-gray-200 rounded-lg p-4 hover:border-blue-300 transition">
                                    ...
                                    @php
                                        $sudahInputSemua = $presensiSudahInputSemua[$jadwal->id] ?? false;
                                        $sudahLewatSemua = $jadwalSudahLewat[$jadwal->id] ?? false;
                                        $isHariIni = ($jadwal->hari === $hariIni);
                                    @endphp
                                    @if($isHariIni)
                                        @if($sudahInputSemua)
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-green-100 text-green-800 mb-2">Sudah diabsen</span>
                                        @elseif($sudahLewatSemua)
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-amber-100 text-amber-800 mb-2">Jadwal lewat — belum diinput</span>
                                        @else
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-600 mb-2">Belum diinput</span>
                                        @endif
                                    @endif
                                    <div class="flex gap-2">
                                        <a href="{{ route('dosen.presensi.create', $jadwal->id) }}"
                                            class="flex-1 inline-flex items-center justify-center px-3 py-1.5 {{ $isHariIni && $sudahInputSemua ? 'bg-gray-500 hover:bg-gray-600' : 'bg-green-600 hover:bg-green-700' }} text-white text-xs font-medium rounded-lg transition">
                                            {{ $isHariIni && $sudahInputSemua ? 'Edit' : 'Input' }}
                                        </a>
                                        <a href="{{ route('dosen.presensi.show', $jadwal->id) }}"
