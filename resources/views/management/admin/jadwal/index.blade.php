<x-admin-layout>
    <x-slot name="title">Manajemen Jadwal Akademik - SI Akademik</x-slot>

    <div class="mb-6">
        <h2 class="text-3xl font-bold text-gray-900">Manajemen Jadwal Akademik</h2>
        <p class="mt-1 text-sm text-gray-600">Kelola jadwal perkuliahan per golongan</p>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-6">
        <form method="GET" action="{{ route('admin.jadwal.index') }}" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div class="md:col-span-2">
                <label for="search" class="block mb-2 text-sm font-medium text-gray-900">Cari Jadwal</label>
            ...
            <!-- Hari Filter -->
            <div>
                <label for="hari" class="block mb-2 text-sm font-medium text-gray-900">Filter Hari</label>
            ...
            <!-- Golongan Filter -->
            <div>
                <label for="id_Gol" class="block mb-2 text-sm font-medium text-gray-900">Filter Golongan</label>
                <select id="id_Gol" name="id_Gol"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                    <option value="">Semua Golongan</option>
                    @foreach($golongans as $gol)
                        <option value="{{ $gol->id_Gol }}" {{ request('id_Gol') == $gol->id_Gol ? 'selected' : '' }}>
                            {{ $gol->nama_Gol }}
                        </option>
                    @endforeach
                </select>
    ...
    <div class="flex items-center justify-between mb-4">
        <div>
            <p class="text-sm text-gray-600">Total: <span class="font-semibold">{{ $jadwal->total() }}</span> jadwal</p>
        </div>
                        ...
                        <tr class="bg-white border-b hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold
                                    @if($j->hari == 'Senin') bg-blue-100 text-blue-800
                                    @elseif($j->hari == 'Selasa') bg-green-100 text-green-800
                                    @elseif($j->hari == 'Rabu') bg-yellow-100 text-yellow-800
                                    @elseif($j->hari == 'Kamis') bg-purple-100 text-purple-800
                                    @elseif($j->hari == 'Jumat') bg-pink-100 text-pink-800
                            ...
                            <td class="px-6 py-4">
                                <div class="flex flex-col">
                                    <span class="font-semibold text-gray-900">{{ $j->matakuliah->Nama_mk }}</span>
                                    <span class="text-xs text-gray-500 font-mono">{{ $j->matakuliah->Kode_mk }} ({{ $j->matakuliah->sks }} SKS)</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="font-medium text-gray-900">{{ $j->ruang->nama_ruang }}</span>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <form action="{{ route('admin.jadwal.destroy', $j) }}" method="POST"
                                        onsubmit="return confirm('Yakin ingin menghapus jadwal {{ $j->matakuliah->Nama_mk }} - {{ $j->hari }}?')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-8 text-center text-gray-500">
                                ...
                                <p class="text-lg font-medium">Tidak ada data jadwal akademik</p>
                                <p class="text-sm">Tambahkan jadwal baru untuk memulai</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
